...
class Protac_AFP(nn.Module):

    def __init__(self, node_feat_size,edge_feat_size,n_layers):
        super(Protac_AFP,self).__init__()
        self.afp = AttentiveFPGNN(node_feat_size=node_feat_size,edge_feat_size=edge_feat_size,num_layers=n_layers,graph_feat_size=2048)
    ...
    def forward(self,g,show=False):
        h=self.afp(g, g.ndata['atomic'], g.edata['atomic'])
        if show=="1":
            print("hook")
