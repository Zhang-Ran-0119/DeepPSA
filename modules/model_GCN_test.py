...
import torch
class Protac_GCN1(nn.Module):

    def __init__(self, in_feats,hidden_feats,dropouts,activation):
        super(Protac_GCN1,self).__init__()
        self.conv = GCN(in_feats=in_feats,hidden_feats=hidden_feats,activation=activation)
    ...
    def get_grad_cam_weights(self,g,output, target_category):
        ...
        node_weights = self.gradients * g.ndata['h']  # Element-wise multiplication
        cam = node_weights.sum(dim=0)  # Sum over all nodes to get a graph-level CAM
        return cam
