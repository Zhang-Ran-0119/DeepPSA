...
def train(model,batch_size, device, trainloader, validloader, lr, epoch,loss_name,writer):
    model = model.to(device)
    opt = torch.optim.Adam(model.parameters(), lr=lr)
    warmupsteps=WARMUP_STEPS
    batch_id = 0
    max_lr = 1e-4
    warmup_lr = torch.optim.lr_scheduler.LambdaLR(optimizer=opt, lr_lambda=lambda epoch: (batch_id / warmupsteps) * (max_lr / lr) if batch_id < warmupsteps else 1)
